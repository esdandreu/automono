...
class EmivasaCostsSource(CostsSource):
    ...
    def __init__(self, username: str, password: str, browser: Browser, logger: Logger):
    ...
    def __iter__(self) -> Iterator[Invoice]:
        """
        Iterate over Emivasa invoices from newest to oldest.
        
        Yields:
            Invoice objects with complete metadata and file content
        """
        try:
            self.logger.info("Starting Emivasa invoice iteration")
            ...
            self.browser.wait_for_element(By.CLASS_NAME, "billing", timeout=30)
            ...
            for metadata in invoice_metadata_list:
                try:
                    ...
                    self.logger.info("Successfully processed Emivasa invoice",
                                   file_name=metadata['filename'],
                    ...
                    self.logger.error("Failed to process Emivasa invoice",
                                    file_name=metadata.get('filename', 'unknown'),
                                    error=str(e))
                    continue
            
        except Exception as e:
            self.logger.error("Failed to iterate Emivasa invoices", error=str(e))
            raise
        finally:
            if self.driver:
                self.browser.stop()
    ...
    def _download_invoice_file(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            self.logger.info("Downloading Emivasa invoice file", 
                           file_name=metadata['filename'])
            ...
            self.logger.info("Successfully downloaded Emivasa invoice file",
                           file_name=metadata['filename'],
                           size=len(content))
            ...
            self.logger.error("Failed to download Emivasa invoice file",
                            file_name=metadata.get('filename', 'unknown'),
                            error=str(e))
            raise
                ...
                try:
                    ...
                    if date_match:
                        ...
                        invoice_data = {
                            ...
                            'filename': f"emivasa_{invoice_date.strftime('%Y%m%d')}.pdf"
                        }
                        invoices.append(invoice_data)
