...
class DigiCostsSourceAdapter(CostsSource):
    ...
    def get_available_invoices(self, since_date: datetime) -> List[InvoiceMetadata]:
        """
        Get available invoices from Digi since the specified date.
        ...
        """
        try:
            self.logger.info("Getting available Digi invoices", since_date=since_date.isoformat())
            ...
            self.logger.info("Successfully retrieved Digi invoices", 
                           count=len(invoices),
            ...
            self.logger.error("Failed to get Digi invoices", error=str(e))
            raise
        finally:
            if self.driver:
    ...
    def download_invoice(self, invoice_metadata: InvoiceMetadata) -> InvoiceFile:
        """
        Download a specific invoice file from Digi.
        ...
        """
        try:
            self.logger.info("Downloading Digi invoice", 
            ...
            self.logger.info("Successfully downloaded Digi invoice",
                           file_name=invoice_metadata.file_name,
                           size=len(content))
            ...
            self.logger.error("Failed to download Digi invoice",
                            file_name=invoice_metadata.file_name,
                            error=str(e))
            raise
                    ...
                    if date_match:
                        ...
                        if invoice_date >= since_date:
                            ...
                            metadata = InvoiceMetadata(
                                ...
                                deductible_percentage=self.DEDUCTIBLE_PERCENTAGE,
                                file_name=f"digi_{invoice_date.strftime('%Y%m%d')}.pdf"
                            )
                            invoices.append(metadata)
